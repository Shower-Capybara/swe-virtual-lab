...
from db_manage.db_models import (
    Base, User, Quiz, QuizQuestion, QuizQuestionOption,
    QuizSubmission, QuizSubmissionAnswer, Challenge,
    ChallengeSubmission, PageView
)
from db_manage.db import engine, async_session_maker
...
class AsyncDatabaseSeeder:
    ...
    async def seed_quizzes(self, num_quizzes: int) -> List[Quiz]:
        ...
        quiz_dates = self.consistency_manager.generate_sequential_dates(
            num_quizzes,
            start_date=datetime.utcnow() - timedelta(days=180),  # Start from 6 months ago
            max_interval_days=5
        )
        ...
        for user in users[:10]:  # First 10 users submit quizzes
            for quiz in quizzes[:5]:  # First 5 quizzes
                ...
                for question in quiz.questions:
                    ...
                    max_time = min(300, (900 - total_time) // max(1, remaining_questions))
